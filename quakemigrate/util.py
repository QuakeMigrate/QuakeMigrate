    ...
    for trace in stream:
        ...
        if sampling_rate != trace_sampling_rate:
            if (trace_sampling_rate % sampling_rate) == 0:
                ...
                upsampled = round(trace_sampling_rate * upfactor)
                if upsampled % sampling_rate != 0:
                    raise ValueError(
                        f"Invalid upfactor={upfactor}: trace_sr={trace_sampling_rate} "
                        f"cannot be resampled to target_sr={sampling_rate} "
                        f"(upsampled_sr={upsampled} not divisible). Trace={trace.id}"
                    )
