...
#   GPL V.3
#
#   Copyright (c) 2017, Coalescence Microseismic Scanning
#
...
# As local time including timezone information
TIMESTAMP0 = datetime.datetime(1970, 1, 1, 0, 0)

# (1981, 6, 30) to (2016, 12, 31)
LEAP_UTC_TICK = np.array([362793599.0, 394329599.0, 425865599.0, 489023999.0, 567993599.0, 631151999.0,
                          662687999.0, 709948799.0, 741484799.0, 773020799.0, 820454399.0, 867715199.0,
                          915148799.0, 1136073599.0, 1230767999.0, 1341100799.0, 1435708799.0, 1483228799.0])


def utc_leapseconds(utc_timestamp):
    return np.sum(LEAP_UTC_TICK < utc_timestamp)


def gps_leapseconds(gps_timestamp):
    leap_gps_tick = LEAP_UTC_TICK + 1 + np.arange(len(LEAP_UTC_TICK))
    return np.sum(leap_gps_tick < gps_timestamp)


def gps2utc(tm):
    return tm - gps_leapseconds(tm)


def utc2gps(tm):
    return tm + utc_leapseconds(tm)


def gpstick_to_time(gps_tick):
    return 315964800.0 + gps_tick/1000000


def gpstime_to_tick(gps_time):
    try:
        return int(1000000*gps_time+0.5) - 315964800
    except TypeError:
        return int(1000000*gps_time.timestamp+0.5) - 315964800


def gpstick_to_utc(gps_tick):
    return gps2utc(gpstick_to_time(gps_tick))


def utc_to_gpstick(utc_time):
    return gpstime_to_tick(utc2gps(utc_time))

# """
# Module containing a UTC-based datetime class.
#    obspy.code.utcdatetime
#
# :copyright:
